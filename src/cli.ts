...
program
	...
	.requiredOption(
		'--file <file>',
		'schema.prisma file location',
		DEFAULT_FILE_LOCATION,
	)
	.option(
		'--table-case <tableCase>',
		'case convention for table names. allowable values: "pascal", "camel", "snake"',
		'pascal',
	)
	.option(
		'--field-case <fieldCase>',
		'case convention for field names. allowable values: "pascal", "camel", "snake"',
		'camel',
	)
	.option(
		'-D, --dry-run',
		'print changes to console, rather than back to file',
		false,
	)
;``
...
async function run() {
	...
	if (error_file) {
		console.error(
			'Encountered an error while trying to read provided schema.prisma file at path ' +
				options.file,
		)
	...
	let [tableCaseConvention, error_table_case] = tryGetTableCaseConvention(
		options.tableCase,
	)
	if (error_table_case) {
		console.warn(
	...
	let [fieldCaseConvention, error_field_case] = tryGetTableCaseConvention(
		options.fieldCase,
	)
	if (error_field_case) {
		console.warn(
	...
	const [schema, schema_error] = migrateCaseConventions(
		file_contents!,
		tableCaseConvention!,
		fieldCaseConvention!,
	)
